    ...
    public boolean executeCommand(String inputLine) {
        ...
        try {
            ...
            if (methodToInvoke.getParameterCount() == 0) {
                ...
                String argument = line[1].trim();
                if (argument.isEmpty()) {
                    System.out.println("Не хватает аргументов для команды: " + command);
                    return false;
                }
                methodToInvoke.invoke(this, argument);
            } else {
                System.out.println("Не хватает аргументов для команды: " + command);
                return false;
            }

            System.out.println("Выполнение команды: " + inputLine);
            recordCommand(inputLine); // Сохраняем только после успешного выполнения команды
            return true;
